    ...
    <table class="min-w-full divide-y divide-gray-200">
            ...
            <th scope="col"
                ...
                <div class="flex justify-end items-center gap-4 mt-4 mb-4 px-4">
                    <a href="{{ route('todo.create') }}" class="relative select-none">
                        <button
                            class="font-sans font-lg text-center uppercase transition-all disabled:opacity-50 disabled:shadow-none disabled:pointer-events-none w-10 h-10 text-lg bg-blue-500 text-white shadow-md shadow-gray-900/10 hover:shadow-lg hover:shadow-gray-900/20 focus:opacity-[0.85] focus:shadow-none active:opacity-[0.85] active:shadow-none rounded-md"
                            type="button">
                            +
                        </button>
                    </a>
        ...
        @foreach($this->todos() as $todo)
            <tr>
                ...
                <td>
                    <div>
                        <label wire:click.debounce="changeState({{ $todo->id }})" class="flex items-center cursor-pointer select-none text-dark">
                            <div class="relative">
                                <input {{ $todo->state ? 'checked': '' }} wire:key="todo-switch-{{ $todo->id }}"
                                       type="checkbox"
                                       class="peer sr-only"
                                />
                                <div class="block h-8 rounded-full w-14 {{ $todo->state ? 'bg-emerald-500' : 'bg-red-500' }} transition-colors duration-300"></div>
                                <div
